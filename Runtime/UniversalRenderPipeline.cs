...
namespace UnityEngine.Rendering.Universal
{
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        (CameraData[] , CameraSetupData[]) SetupCamera(Camera[] cameras) {
            ...
            //determine buffer operation based on camera config/relation
            for (int i = 0; i < cameras.Length; ++i) {
                var camera = cameras[i];
                ...
                //resolveFinalTarget value is filled when actually rendering
                //don't want to move the (rather long) checking logic around
                //set to true for scene view camera
                InitializeCameraData(camera, additionalCameraData, true, out CameraDataBuffer[i]);
        ...
        {
            RenderSingleCamera(context, cameraData, ref lastCameraData, cameraData.postProcessEnabled);
        }
        ...
        static void RenderSingleCamera(ScriptableRenderContext context, CameraData cameraData, ref CameraSetupData cameraSetup, bool anyPostProcessingEnabled)
        {
            ...
            using (new ProfilingScope(cmd, sampler))
            {
                ...
                InitializeRenderingData(asset, ref cameraData, ref cameraSetup, ref cullResults, anyPostProcessingEnabled, out var renderingData);
        ...
        static void RenderCameraStack(ScriptableRenderContext context, Camera baseCamera, ref CameraData baseCameraData, ref CameraSetupData baseCameraSetup)
        {
            ...
            baseCameraData.resolveFinalTarget = !isStackedRendering;
            //InitializeCameraData(baseCamera, baseCameraAdditionalData, !isStackedRendering, out var baseCameraData);
            RenderSingleCamera(context, baseCameraData, ref baseCameraSetup, anyPostProcessingEnabled);
            ...
            for (int i = 0; i < cameraStack.Count; ++i)
            {
                ...
                if (currCameraData != null)
                {
                    ...
                    RenderSingleCamera(context, overlayCameraData, ref baseCameraSetup, anyPostProcessingEnabled);
        ...
        static void InitializeStackedCameraData(Camera baseCamera, UniversalAdditionalCameraData baseAdditionalCameraData, ref CameraData cameraData)
        {
            ...
            cameraData.renderScale = Mathf.Max(kRenderScaleThreshold,
                                                    (baseAdditionalCameraData != null && baseAdditionalCameraData.overrideRenderScale) ? baseAdditionalCameraData.renderScale :
                                                    (baseCamera.cameraType == CameraType.Game) ? usedRenderScale : 1.0f);
...
#pragma warning restore 0618
        ...
        static void InitializeRenderingData(UniversalRenderPipelineAsset settings, ref CameraData cameraData, ref CameraSetupData cameraSetup,
            ref CullingResults cullResults, bool anyPostProcessingEnabled, out RenderingData renderingData)
        {
            ...
            if (cameraData.maxShadowDistance > 0.0f)
            {
                ...
                if (settings.additionalLightsRenderingMode == LightRenderingMode.PerPixel)
                {
                    for (int i = 0; i < visibleLights.Length; ++i)
                    {
                        ...
                        var supported = light.type == LightType.Spot || light.type == LightType.Directional;
                        if (supported && light != null && light.shadows != LightShadows.None)
                        {
